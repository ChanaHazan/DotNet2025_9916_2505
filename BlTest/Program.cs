...
internal class Program
{
    ...
    {
        // 1. אתחול אובייקט הזמנה עם רשימה ריקה
            ...
            try
            {
                switch (choice)
                {
                    case 1:
                        ...
                        // הקריאה ל-BL - אנחנו תופסים את רשימת המבצעים שחזרה
                        ...
                        // הצגת המבצעים (התוספת שדיברנו עליה)
                        ...
                        Console.WriteLine($"Current Total: {currentOrder.TotalPrice:C}");
                        break;

                    case 2:
                        if (!currentOrder.ProductInOrder.Any())
                        {
                        ...
                        return; // סיום וחזרה לתפריט ראשי

                    case 3:
                        // הדפסה עשירה בזכות ה-ToStringProperty
                        Console.WriteLine("\n--- Current Order Status ---");
                        Console.WriteLine(currentOrder);
                        break;
                }
            }
            catch (BlInputNotCorectException ex) // שגיאה שלך
            {
                Console.WriteLine($"Input Error: {ex.Message}");
            }
            catch (BLThereIsNotEnoughInStock ex) // שגיאה שלך - קריטי להזמנה!
            {
